...
if (isset($_GET['complete_order'])) {
    $completeId = (int)$_GET['complete_order'];
    $stmt = $conn->prepare("UPDATE orders SET status = 'completed' WHERE id = ? AND client_id = ? AND status != 'completed' AND status != 'cancelled'");
    $stmt->bind_param("ii", $completeId, $userId);
    $stmt->execute();
    $stmt->close();
                            ...
                            <tr class="hover:bg-gray-50 even:bg-white odd:bg-gray-50">
                                ...
                                <td class="px-4 py-3 border-b border-gray-200 space-x-2">
                                    ...
                                    <?php if (!in_array($row['status'], ['completed', 'cancelled'])): ?>
                                        <a href="?complete_order=<?= (int)$row['id'] ?>"
