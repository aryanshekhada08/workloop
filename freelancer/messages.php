...
// Ensure logged in and role is freelancer
...
// Default profile image filename (change as per your setup)
$defaultImage = "default.png";

// Query to get client contacts with last message info for this freelancer
$sql = "
    ...
    <main class="ml-80 p-6 max-w-4xl mx-auto">
        ...
        <?php if ($result->num_rows > 0): ?>
            <ul class="divide-y divide-gray-200 bg-white rounded shadow-md">
                ...
                ?>
                <li>
                    <a href="chat.php?user_id=<?= (int)$row['client_id'] ?>" class="block p-4 hover:bg-green-50 flex justify-between items-center gap-4">
                        <div class="flex items-center gap-3">
                            <?php if ($hasImage): ?>
                                <img src="<?= htmlspecialchars($imgPath) ?>" alt="<?= htmlspecialchars($row['client_name']) ?>"
                            ...
                            <?php else: ?>
                                <div class="w-10 h-10 rounded-full bg-green-700 flex items-center justify-center text-white font-semibold select-none">
                                    <?= strtoupper(htmlspecialchars(substr($row['client_name'], 0, 2))) ?>
                                </div>
                            <?php endif; ?>
                            <div>
